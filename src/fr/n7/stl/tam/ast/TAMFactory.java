...
public interface TAMFactory {
	...
	public static TAMInstruction createMemoryOperator(MemoryOperator _operator) {
		switch (_operator) {
		case Allocation : return Library.MAlloc;
		case Liberation : return Library.MFree;
		default: throw new SemanticsUndefinedException("Unexpected memory operator: " + _operator);
		}
	}
	
	
	public static TAMInstruction createPrintOperation(Type type) {
		if(type.equals(AtomicType.BooleanType))
			return Library.BOut;
		if(type.equals(AtomicType.IntegerType))
			return Library.IOut;
		if(type.equals(AtomicType.CharacterType))
			return Library.COut;
		if(type.equals(AtomicType.StringType))
			return Library.SOut;
		...
		throw new SemanticsUndefinedException("Unexpected type to print " + type);
	}
